...
for i in range(n):
    ...
    M = AffineLucasKanade(It, It1, rect, threshold, num_iters)
    p = M.flatten()[:6]
    ...
    # plt.show()
    plt.savefig("../dataset/coke_results_affine/coke_tracking%02d.jpg" % i)
    iou_list.append(compute_iou(gt[i], rect))

np.save("../result/affinecokeseqrects.npy",cokeseqrects)
